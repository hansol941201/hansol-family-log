window.PDFExport={async run(start,end){
  ...
  const w=open('','_blank');w.document.write(`<!doctype html><html lang="ko"><head><title>한솔기록 ${start}~${end}</title><style>@page{size:A4;margin:16mm}body{font-family:Arial,sans-serif;color:#17212b;font-size:12px}header{border-bottom:2px solid #102a43;margin-bottom:18px}h1{font-size:22px;margin-bottom:5px}header p{color:#667}section{break-inside:avoid;margin:0 0 20px}h2{font-size:15px;color:#102a43}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccd5dc;padding:7px;text-align:left;vertical-align:top}th{width:25%;background:#f1f4f6}.attachments{display:flex;flex-wrap:wrap;gap:8px;margin-top:8px}figure{margin:0;width:150px}img{max-width:150px;max-height:150px;object-fit:contain}figcaption,.file{font-size:10px;color:#667;margin-top:3px}</style></head><body><header><h1>한솔기록</h1><p>${start} ~ ${end} · ${days.length}일</p></header>${sections}</body></html>`);w.document.close();setTimeout(()=>w.print(),500)
}};
